...
function NavbarLayout(props) {
  ...
  const checkRole = (role) => {
    if (role === 3) history.push("/student");
    if (role === 2) history.push("/teacher");
  };
  return (
    <Navbar bg="light" expand="lg" className="navbar__custom">
      ...
      <Navbar.Collapse id="basic-navbar-nav" className="navbar__right">
        ...
        {token && (
          ...
          >
            <NavDropdown.Item onClick={() => checkRole(userInfo.role)}>
              Profile
            </NavDropdown.Item>
            <NavDropdown.Divider />
            {userInfo.role === 2 && (
            ...
            {userInfo.role === 2 && (
              <NavDropdown.Item
                onClick={() => history.push("/teacher/courses/add-course")}
              >
                Add Course
              </NavDropdown.Item>
            )}
            ...
            {userInfo.role === 3 && (
              <NavDropdown.Item
                onClick={() => history.push("/student/wishlist")}
              >
                Wishlist
              </NavDropdown.Item>
            )}
